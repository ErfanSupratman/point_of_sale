...
class History_stock_model extends CI_Model {
	...
	function getHistoryStockByStockId($draw, $stockId, $start, $limit, $search) {
		...
		$dataWrapper->recordsFiltered = $this->countAllHistoryByStockIdAndFIlter($stockId, $search)->total;
	...
	function countAllHistoryByStockIdAndFIlter($stockId, $search) {
		$sql = 'SELECT count(*) as total  FROM pos_history_stock WHERE stock_id=? 
		AND 
		(CONVERT(stock_out USING utf8) LIKE ? 
			OR CONVERT(stock_in USING utf8) LIKE ? 
			OR CONVERT(notes USING utf8) LIKE ? 
			OR CONVERT(created_date USING utf8) LIKE ?
			 OR CONVERT(created_by USING utf8) LIKE ?) 
		ORDER BY created_date';
		$query = $this->db->query($sql, array($stockId, '%'.$search.'%', '%'.$search.'%', '%'.$search.'%', '%'.$search.'%', '%'.$search.'%'));
		return $query->row();
	}
